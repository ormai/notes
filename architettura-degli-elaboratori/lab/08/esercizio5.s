...
main:
            ...
            // call collatz(n) and print its return value
            ldr         w0, n
            bl          collatz
            mov         w1, w0
            adr         x0, fmt
...
if:         tst         w0, #1 // if w0 is odd goto else
            ...
            // collatz(n // 2) + 1
            mov         w1, #2 // udiv doesn't take immediates
            udiv        w0, w0, w1
            bl          collatz // recursive call
            add         w0, w0, #1

            b           return

else:       // return collatz(n * 3 - 1) + 2
            mov         w1, #3 // madd doesn't take immediates
            mov         w2, #1 // ^^^
            madd        w0, w0, w1, w2 // w0 = w2 + (w0 * w1) = 1 + (n * 3)
            bl          collatz // other recursive call
            add         w0, w0, #1
