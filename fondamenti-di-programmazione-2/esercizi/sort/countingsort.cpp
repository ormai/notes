...
void countingSort(int input[], const unsigned size) {
   ...
   *
   * Esempio: cont = [1, 1, 3, 3, 5, ...], il terzo elemento c'è due volte. */
  for (unsigned i = 1; i <= max; ++i)
    count[i] += count[i - 1];

  /* Le chiavi dell'array di input sono usate come indici per accedere agli
   * indici appena calcolati. Tali indici in count sono usati per posizionare
   * le chiavi (input[i]) nelle correte posizioni all'interno dell'array di
   * output, ovvero: count[input[i]] è l'indice calcolato nello scorso loop, ed
   * è usato per accedere ad un elemento di count. Dopo che l'indice contenuto
   * in count è usato, questo viene decrementato, così una stessa chiave sarà
   * posizionata immediatamente a sinistra. Poter gestire correttamente le
   * ripetizioni delle chiavi è il motivo per cui l'operazione è fatta dalla
   * fine all'inizio. */
